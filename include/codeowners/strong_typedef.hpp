...
template <typename Tag, typename T> struct strong_typedef
{
    using value_type = T;
...
template <typename StrongTypedef> using underlying_type_t = typename StrongTypedef::value_type;

template <typename S> struct equality_comparable : boost::equality_comparable<S>
{
    friend bool operator==(const S& a, const S& b) { return a.value() == b.value(); }
};

template <typename S, typename U>
struct underlying_equality_comparable : equality_comparable<S>, boost::equality_comparable<S, U>
{
    friend bool operator==(const S& s, const U& u) { return s.value() == u; }
};

template <typename S> struct less_than_comparable : boost::less_than_comparable<S>
{
    friend bool operator<(const S& a, const S& b) { return a.value() < b.value(); }
};

template <typename S> struct ostreamable
{
...
template <typename S> struct istreamable
{
...
template <typename S> struct streamable : ostreamable<S>, istreamable<S>
{
};
