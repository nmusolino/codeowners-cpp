...
/// string.
struct owner : public strong_typedef<owner, std::string>,
...
/// string.
struct pattern : public strong_typedef<pattern, std::string>,
...
struct rule_source
{
    ...
    {
        return std::tie(lhs.filename, lhs.line) == std::tie(rhs.filename, rhs.line);
    }
...
struct annotated_rule
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const annotated_rule& ann_rule)
    {
