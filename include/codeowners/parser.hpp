...
std::optional<ownership_rule>
parse_line(const std::string& line);

std::vector<annotated_rule>
parse(std::istream& is, const std::string& source_name = "");

std::vector<annotated_rule>
parse(const fs::path& source, const std::string& source_name = "");

template <typename InputIt>
std::vector<annotated_rule>
parse(InputIt begin, InputIt end, const std::string& source_name = "")
{
    auto make_annotated_rule =
      [&source_name](const std::pair<std::size_t, ownership_rule>& pair) {
          const auto& [i, rule] = pair;
          // Use `i+1`, since line numbers conventionally begin at 1.
          return annotated_rule{
            rule_source{source_name, static_cast<std::int32_t>(i + 1)}, rule};
      };
